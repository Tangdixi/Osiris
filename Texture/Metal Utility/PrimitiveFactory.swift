...
class PrimitiveFactory {
    ...
    class func makeSphere(device: MTLDevice, extent: vector_float3, segments: vector_uint2) -> MDLMesh {
        let allocator = MTKMeshBufferAllocator(device: device)
        let mesh = MDLMesh(sphereWithExtent: extent, segments: segments, inwardNormals: false, geometryType: .triangles, allocator: allocator)
        return mesh
    }
