...
extension ViewController {
    ...
    func setup() {
        ...
        if models.count == 0 {
            ...
            models.append(contentsOf: [train, tree])
        }
        ...
        renderer.draw = { (renderCommandEncoder) in
            ...
            self.timer += 0.02
            var lights = [Light]()
            var sunLight = LightFactory().sunLight
            sunLight.position = [sin(self.timer)*3, 1.5, -3]
            
            let ambientLight = LightFactory().ambientLight
            lights.append(contentsOf: [sunLight, ambientLight])
